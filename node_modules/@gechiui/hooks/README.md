...
```bash
npm install @gechiui/hooks --save
```
...
In the GeChiUI context, that enables API functions to be called via the global `gc.hooks` object, like `gc.hooks.addAction()`, etc.
